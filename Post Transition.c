...
void send_all_acceptable_packages(town* source, int source_office_index, town* target, int target_office_index) {	
    int min, max, target_pkg_count, source_pkg_count;
    ...
    for(int i = 0; i < source->offices[source_office_index].packages_count; i++){
		int pkg_weight = source->offices[source_office_index].packages[i].weight;
        if((pkg_weight >= min) && (pkg_weight) <= max){
			target_pkg_count = target->offices[target_office_index].packages_count;

            target->offices[target_office_index].packages = realloc(target->offices[target_office_index].packages, (target_pkg_count+1)*sizeof(package));

            target->offices[target_office_index].packages[target_pkg_count] = source->offices[source_office_index].packages[i];

			source_pkg_count = source->offices[source_office_index].packages_count;
            for(int j = i; j < source_pkg_count; j++){
                source->offices[source_office_index].packages[j] = source->offices[source_office_index].packages[j+1];
            }

            source->offices[source_office_index].packages = realloc(source->offices[source_office_index].packages, (source_pkg_count-1) * sizeof(package));
